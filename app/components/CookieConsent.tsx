...
export default function CookieConsent() {
  ...
  // Implied consent on navigation AWAY while banner visible and no consent yet
  useEffect(() => {
    if (!show) return
    const c = getConsent()
    if (!c) {
  ...
  // Re-prompt on HOME if consent exists but is NOT explicit
  useEffect(() => {
    const c = getConsent()
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-50 border-t border-[color:var(--line)] bg-white/97 backdrop-blur shadow-[0_-10px_20px_rgba(0,0,0,0.08)]">
      <div className="container px-5 py-4 md:py-5 pb-[calc(1rem+env(safe-area-inset-bottom))]">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          {/* Copy */}
          <div className="flex items-start gap-3 md:pr-4">
            <svg aria-hidden="true" width="24" height="24" viewBox="0 0 24 24" className="shrink-0 text-[color:var(--brand)] mt-0.5">
            ...
            <div>
              ...
              <p className="text-sm text-[color:var(--muted)] leading-relaxed">
                To improve your experience, analyze site traffic, and support marketing. Read our{' '}
                <a href="/privacy-policy" className="underline text-[color:var(--brand)]">Privacy Policy</a>.
              </p>
            </div>
          </div>
          ...
          <div className="w-full md:w-auto">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
              <button
                onClick={rejectAll}
                className="h-11 rounded-md border border-[color:var(--line)] px-4 font-medium w-full hover:bg-[color:var(--surface)]"
              >
                Reject All
              </button>
              <button
              ...
              <button
                onClick={confirmAll}
                className="h-11 rounded-md px-5 font-semibold text-white w-full"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3 w-full sm:w-auto">
                <button
                  ...
                  className="h-10 rounded-md border border-[color:var(--line)] px-4 font-medium w-full"
                >
                  Back
                </button>
                <button
                  onClick={savePrefs}
                  className="h-10 rounded-md px-4 font-semibold text-white w-full"
                  style={{ backgroundColor: 'var(--brand)' }}
                >
                  Save
                </button>
